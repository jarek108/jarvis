...
def run_llm_comparison():
    ...
    print("#"*LINE_LEN)
    print(f"{BOLD}{CYAN}{'LLM MULTI-LOADOUT COMPARISON BENCHMARK':^120}{RESET}")
    print("#"*LINE_LEN)
    ...
    print("\n" + "="*LINE_LEN)
    print(f"{BOLD}{'LLM MULTI-LOADOUT COMPARISON REPORT (SPEED & MEMORY)':^120}{RESET}")
    print("="*LINE_LEN)

    for name in all_scenario_names:
        print(f"\n{BOLD}Scenario: {name}{RESET}")
        print(f"  {'Loadout':<40} | {'TTFT':<8} | {'TPS':<6} | {'VRAM Peak':<10} | {'Placement'}")
        print(f"  {'-'*40} | {'-'*8} | {'-'*6} | {'-'*10} | {'-'*15}")
        ...
        for lid in loadouts:
            ...
            if s_res and s_res['status'] == "PASSED":
                ...
                print(f"  {lid:<40} | {s_res['ttft']:.3f}s | {s_res['tps']:>5.1f} | {v_peak:<10} | {placement}")
            else:
