...
from utils import format_status, CYAN, BOLD, RESET, LINE_LEN, RED, list_all_tts_models

def run_tts_suite():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    # DYNAMIC DISCOVERY
    models = list_all_tts_models()
    ...
    print("#"*LINE_LEN)
    print(f"{BOLD}{CYAN}{'TTS MULTI-MODEL ONE-TO-ONE COMPARISON':^120}{RESET}")
    print("#"*LINE_LEN)
    ...
    print("\n" + "="*LINE_LEN)
    print(f"{BOLD}{'TTS MULTI-MODEL CONSOLIDATED PERFORMANCE REPORT':^120}{RESET}")
    print("="*LINE_LEN)
    
    for name in all_scenario_names:
        print(f"\n{BOLD}Scenario: {name}{RESET}")
        print(f"  {'Model':<30} | {'Status':<8} | {'Time':<8} | {'Resulting Path'}")
        print(f"  {'-'*30} | {'-'*8} | {'-'*8} | {'-'*60}")
