...
def test_isolated():
    cfg = load_config()
    target_id = "chatterbox-multilingual"
    ...
    print(f"\n--- SMART TTS {target_id.upper()} LIFECYCLE (Port {port}) ---")
    ...
    if status == "ON":
        ...
        print(f"STARTING TTS Server with {target_id} Model (Quietly)...")
        cmd = [python_exe, server_script, "--port", str(port), "--variant", target_id]
        process = start_server(cmd, loud=False)
        started_here = True
        if not wait_for_port(port, timeout=180, process=process):
    ...
    LINE_LEN = 120
    print("\n" + "="*LINE_LEN)
    print(f"{BOLD}{CYAN}{'TTS ISOLATED TEST: ' + target_id.upper():^120}{RESET}")
    print("="*LINE_LEN)
    
    f = LiveFilter()
    proc_start = time.perf_counter()
    with redirect_stdout(f):
        run_test(variant_id=target_id)
