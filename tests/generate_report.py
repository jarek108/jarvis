...
def generate_excel(upload=True, upload_outputs=False, session_dir=None):
    try:
        ...
        COLUMN_DEFINITIONS = {
            "Loadout": "Combination of models and engines used in this run.",
            "Scenario": "Specific test case or prompt executed.",
            "Status": "Final outcome: PASSED, FAILED, or MISSING (models not found).",
            "Exec": "[Total Execution Time] Duration of the primary inference or pipeline call (s).",
            "Setup": "[Initialization] Time taken to boot servers, load models into VRAM, and perform warmups (s).",
            "Cleanup": "[Teardown] Time taken to kill processes and release GPU memory (s).",
            "VRAM": "[Peak Usage] Highest recorded GPU memory consumption during the run (GB).",
            "TTFT": "[Time To First Token] Latency from request start until the first piece of data is received (s).",
            "TPS": "[Tokens Per Second] Average generation speed of the LLM (tokens/s).",
            "RTF": "[Real-Time Factor] Formula: Execution Time / Audio Duration. (Lower is better).",
            "WPS": "[Words Per Second] Formula: Total Words / Execution Time.",
            "CPS": "[Characters Per Second] Formula: Total Characters / Execution Time.",
            "Match %": "[Similarity] Fuzzy matching score between transcription and ground truth.",
            "STT Inf": "[STT Latency] Time spent specifically on Speech-to-Text inference (s).",
            "LLM Tot": "[LLM Latency] Total time spent on LLM processing (Prompt + Generation) (s).",
            "TTS Inf": "[TTS Latency] Time spent specifically on Text-to-Speech inference (s).",
            "Audio": "Link to the input or output audio file on Google Drive.",
            "Media": "Link to the input image or video file on Google Drive.",
            "Prompt": "The text or instruction sent to the model.",
            "Response": "The raw text generated by the model.",
            "Result": "The transcription or final processed output text."
        }
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            for name, df in sheets.items():
                ...
                for cell in worksheet[1]: 
                    ...
                    if cell.value in COLUMN_DEFINITIONS:
                        cell.comment = Comment(COLUMN_DEFINITIONS[cell.value], "Jarvis")
