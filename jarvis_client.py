...
class JarvisApp(ctk.CTk):
    ...
    def _update_selection_ui(self):
        for mid, widgets in self.service_widgets.items():
            if mid == self.selected_mid: widgets['frame'].configure(border_width=2, border_color=self.colors.get('success'), fg_color="#1A202C")
            else: widgets['frame'].configure(border_width=0, fg_color="#12161E")
    ...
    def update_health_ui(self, health, runnability, active_models=None, vram=None):
        if vram:
            ...
            ext_w = min(bar_max_w, ext_w); model_w = min(bar_max_w - ext_w, model_w)
            self.vram_bar_ext.configure(width=ext_w); self.vram_bar_model.place(x=ext_w, y=0); self.vram_bar_model.configure(width=model_w)
        ...
        for mid in list(self.service_widgets.keys()):
            if mid not in current_mids: self.service_widgets[mid]['frame'].destroy(); del self.service_widgets[mid]
        ...
        for m in models_to_render:
            ...
            if mid not in self.service_widgets:
                ...
                f.pack(fill="x", pady=6, padx=5); f.bind("<Button-1>", on_click); f.bind("<Button-3>", on_rclick)
                f.bind("<Enter>", lambda e, frame=f, m_id=mid: on_enter(e, frame, m_id)); f.bind("<Leave>", lambda e, frame=f, m_id=mid: on_leave(e, frame, m_id))
                header = ctk.CTkFrame(f, fg_color="transparent"); header.pack(fill="x", padx=8, pady=(8, 0)); header.bind("<Button-1>", on_click); header.bind("<Button-3>", on_rclick)
                lamp = ctk.CTkLabel(header, text="●", font=("Arial", 18)); lamp.pack(side="left", padx=(0, 5)); lamp.bind("<Button-1>", on_click); lamp.bind("<Button-3>", on_rclick)
                ...
                name_box.insert("1.0", mid); name_box.configure(state="disabled"); name_box.pack(side="left", fill="x", expand=True); name_box.bind("<Button-1>", on_click); name_box.bind("<Button-3>", on_rclick)
                caps = m.get('capabilities', []); inputs = [c.replace("_in", "") for c in caps if c.endswith("_in")]; outputs = [c.replace("_out", "") for c in caps if c.endswith("_out")]
                subtext = ctk.CTkLabel(f, text=f"IN: {', '.join(inputs)} | OUT: {', '.join(outputs)}", font=("Consolas", 10), text_color="#D0D0D0", anchor="w")
                subtext.pack(fill="x", padx=28, pady=(0, 2)); subtext.bind("<Button-1>", on_click); subtext.bind("<Button-3>", on_rclick)
                stream_frame = ctk.CTkFrame(f, fg_color="transparent"); stream_frame.pack(fill="x", padx=28, pady=(0, 2)); stream_frame.bind("<Button-1>", on_click); stream_frame.bind("<Button-3>", on_rclick)
                is_llm = m['engine'] in ['ollama', 'vllm']; streaming = m.get('params', {}).get('stream', True if is_llm else False)
                e_lbl = ctk.CTkLabel(stream_frame, text=f"{m['engine'].upper()} • Out-Stream: ", font=("Consolas", 10), text_color="#B0B0B0"); e_lbl.pack(side="left"); e_lbl.bind("<Button-1>", on_click); e_lbl.bind("<Button-3>", on_rclick)
                s_lamp = ctk.CTkLabel(stream_frame, text="●", font=("Arial", 12), text_color=(self.colors.get('success') if streaming else self.colors.get('error'))); s_lamp.pack(side="left"); s_lamp.bind("<Button-1>", on_click); s_lamp.bind("<Button-3>", on_rclick)
                params_dict = m.get('params', {}).copy(); params_dict.pop('device', None); params_dict.pop('stream', None)
                if params_dict:
                    ...
                    params_box.insert("1.0", p_str); params_box.configure(state="disabled"); params_box.pack(fill="x", padx=28, pady=(0, 8)); params_box.bind("<Button-1>", on_click); params_box.bind("<Button-3>", on_rclick)
                else: ctk.CTkLabel(f, text="", height=4).pack()
